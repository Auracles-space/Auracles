...
| Internal rarity | `1 - max_jaccard_to_any_existing_published_artifact` | LSH query on Redis-backed index. Updated when an artifact reaches `published` state (never on draft / failed). |
...
**Add:**
- `app/workers/tasks/processing/metadata.py`: completes `metadata_vector` JSONB → adds `top_tfidf_terms[20]` (via `sklearn.feature_extraction.text.TfidfVectorizer` fit incrementally per worker startup against running published-artifact corpus; persisted as joblib pickle in Redis), `language` (via `langdetect`), tag overlap counts.
...
- `app/workers/tasks/processing/rarity_internal.py`: `compute_internal_rarity(artifact_id)` → query `datasketch.MinHashLSH` (Redis backend) over **published artifacts only** for nearest matches; compute `max_jaccard` → `internal_rarity = 1 - max_jaccard`; persist + write `artifact_rarity_audit` row (internal section). On publish (Slice 9), artifact is inserted into LSH index.
