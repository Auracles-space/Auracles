...
- Service: `unpublish_framework(framework_id)` (FR-FWK-009) → sets `status=unpublished`. Product copy must call this **Delist**, not "unpublish": it removes the Framework from new marketplace discovery/purchases only. Existing licensees keep download access (BR check on download stays license-status, not framework-status).
...
- `frontend/src/components/modules/frameworks/*`: `FrameworkForm`, `ArtifactUploader` (drag-drop + progress + scan/processing/PII/rarity badges), `PipelineStatusPanel` (per-check green/red w/ explanation tooltip), `PublishButton` (enabled only when all green or soft-fail acknowledged), `DelistButton` + confirmation modal (calls `POST /v1/frameworks/{id}/unpublish`, but UI text says **Delist from marketplace** and explicitly says existing licensees keep access), `VersionRadios` w/ prior-version display, `SoftFailAcknowledgement` modal, `PiiReviewResolution` modal.
...
- playwright e2e `framework-publish.spec.ts`: Contributor → KYC-verified → create draft → upload PDF → pipeline runs → all green → Publish → catalog shows it → Delist from marketplace → catalog hides it while existing licensed Operator can still download → audit row exists.
