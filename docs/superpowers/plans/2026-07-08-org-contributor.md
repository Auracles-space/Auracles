...
- Produces `UserRole.source: str` **NOT NULL** (`'self'` | `'derived'`). **The `user_roles` uniqueness changes from `(user_id, role)` to `(user_id, role, source)`** so a user may hold the same role both self-selected and org-derived as two independent rows. Without this, self+derived contributor cannot coexist and the "sync never touches self rows" rule is unsatisfiable.
- Re-shapes `sync_derived_roles(db, *, user_id)` from attestor-only to a capability→role map: `_DERIVED_ROLE_MAP = {"attestor": "attestor", "contributor": "contributor"}`. For each `(capability, role)`: `should_have` = user belongs to ≥1 live (not suspended/deactivated) org whose capability is `active`; the SELECT/`with_for_update` lock **filters `role == role AND source == "derived"`**; grant creates `UserRole(role=role, source="derived", approved_at=now)`; revoke deletes ONLY that derived row. **A `source="self"` row is never selected, granted, touched, or deleted here.** Idempotent.
- **`UserRole` query-shape sweep:** with two possible rows per `(user_id, role)` for `contributor`, any `select(UserRole).where(... role == ...)` that expects at most one row and calls `.scalar()` can now raise `MultipleResultsFound`. Grep every `UserRole` query (`grep -rn "UserRole" backend/app`) and make role-existence checks tolerant: `.limit(1)` / `exists()` / explicit `source` filter. RBAC `require_role` passes if ANY row exists. (Attestor stays single-row — derived-only, retired from self-selection — so only `contributor` is affected, but fix the shape everywhere.)

- [ ] **Step 1: Failing unit tests** — (a) `OrgContributorProfile`/`OrgLegalProfile` round-trip; profile `active` defaults true, `verification_level` defaults 1. (b) `sync_derived_roles` grants a `source="derived"` `contributor` role when the user is in an org with `contributor` capability active. (c) a user who holds BOTH a `source="self"` and a `source="derived"` `contributor` row keeps the `self` row after leaving every active-contributor org (sync deletes only the derived row); the two rows coexist under the new uniqueness. (d) `require_role("contributor")` passes when only one of the two rows exists. (e) attestor derived-role behavior still works (regression, single-row).
...
- [ ] **Step 3: Add models + `UserRole.source` + rewrite `sync_derived_roles` + query-shape sweep.** Migration order: (1) add `user_roles.source` nullable; (2) backfill — `UPDATE user_roles SET source='derived' WHERE role='attestor'` (org-derived-only post sub-project 2), `SET source='self'` WHERE `source IS NULL`; (3) `ALTER COLUMN source SET NOT NULL`; (4) drop `uq_user_roles_user_role`, add `UniqueConstraint(user_id, role, source, name="uq_user_roles_user_role_source")`. Then create both new tables (unique on `org_id`). Migration docstring states the WHY + spec reference. Downgrade reverses (restore old unique after collapsing any duplicate rows — dev DB: document that a self+derived pair cannot round-trip to the single-row constraint and the downgrade keeps the `self` row).
