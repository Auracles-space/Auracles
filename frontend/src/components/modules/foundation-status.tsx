...
export function FoundationStatus({ health }: FoundationStatusProps) {
  ...
  return (
    <main className="min-h-screen bg-canvas px-5 py-10 text-primary md:px-10 md:py-16 lg:py-24">
      <section className="mx-auto grid w-full max-w-[1280px] gap-8 md:grid-cols-[280px_1fr]">
        <aside className="rounded-card border border-border bg-sidebar p-5">
          <div className="font-heading text-lg font-semibold">Auracles</div>
          <div className="mt-1 text-sm leading-6 text-secondary">
            Phase 0 foundation
          </div>
          <div className="mt-6 border-t border-border pt-5 text-xs uppercase tracking-[0.05em] text-muted">
            Platform readiness
          </div>
        </aside>

        <div className="rounded-card border border-border bg-surface p-5 md:p-8">
          <div className="flex flex-col gap-4 border-b border-border pb-6 md:flex-row md:items-start md:justify-between">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.05em] text-muted">
                System status
              </p>
              <h1 className="mt-3 max-w-3xl font-heading text-3xl font-semibold text-primary md:text-5xl">
