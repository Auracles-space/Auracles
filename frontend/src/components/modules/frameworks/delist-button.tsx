 ...
 */
export function DelistButton({ api, frameworkId, onCompleted }: DelistButtonProps) {
  ...
  async function handleDelist() {
    ...
    setError(null);
    try {
      await api.unpublish(frameworkId);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Delist failed.");
      setBusy(false);
      return;
    }
    setBusy(false);
