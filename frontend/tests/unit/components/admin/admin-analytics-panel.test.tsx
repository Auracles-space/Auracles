...
describe("AdminAnalyticsPanel", () => {
  ...
  it("renders when no snapshot history has been frozen yet", async () => {
    // A fresh environment has no snapshots, so the dashboard arrives with
    // trend: [] — every chart path must degrade to the empty state rather than
    // throw, which would take down the whole route with a client-side error.
    vi.mocked(
