"""DB-backed factor aggregator test for the attestor_org subject type.

Builds an org attestor profile, stood org attestations, ratings, and org
warnings inline, then checks the rating average and reliability-penalty factors.
The reviewing member who staffed each review never enters the score.
"""
...
async def clean(migrated_database: None) -> AsyncIterator[None]:
    """Reset org-attestor-factor rows in FK-safe order around each test."""
...
async def _seed_org_attestor(
    *, ratings: list[int], warnings: int, late: int
) -> UUID:
    """Create an org attestor with stood attestations, ratings, and warnings."""
    async with async_session_factory() as session:
...
async def test_org_rating_factor_is_normalized_average(clean: None) -> None:
    """rating = (avg_stars - 1)/4 with evidence = number of org ratings."""
    del clean
    org_id = await _seed_org_attestor(ratings=[5, 5, 4], warnings=0, late=0)

    async with async_session_factory() as session:
        cfg = await load_config(session, subject_type="attestor_org")
        result = await factors.org_attestor_factors(session, org_id, cfg)
...
async def test_org_reliability_penalty_from_warnings_and_late(clean: None) -> None:
    """reliability = max(0, 1 - 0.10*(org warnings + late_submission_count))."""
    del clean
    org_id = await _seed_org_attestor(ratings=[4, 4], warnings=2, late=1)
...
async def test_org_no_ratings_scores_zero_without_error(clean: None) -> None:
    """An org attestor with no ratings gets rating value 0 and evidence 0."""
    del clean
    org_id = await _seed_org_attestor(ratings=[], warnings=0, late=0)
