"""Integration tests for org operator capability activation endpoints."""
...
def override_redis() -> Iterator[FakeRedis]:
    """Install a fake Redis for operator capability rate limits."""
...
async def test_activate_operator_capability_happy_path(
    client: AsyncClient,
    ...
    assert body["capability"] == "operator"
...
async def test_activate_operator_capability_rejects_suspended_org(
    client: AsyncClient,
    override_redis: FakeRedis,
    clean_orgs: None,
    migrated_database: None,
...
async def test_admin_operator_capability_status_routes(
    client: AsyncClient,
    override_redis: FakeRedis,
    clean_orgs: None,
    migrated_database: None,
) -> None:
    """Platform admins can suspend, reinstate, and revoke operator capability."""
    del override_redis, clean_orgs, migrated_database
    ...
    reinstated = await client.post(
        f"/v1/admin/orgs/{org['id']}/operator-capability/reinstate",
        headers=admin_headers,
    )
